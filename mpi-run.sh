#!/bin/bash
/usr/sbin/sshd -f /home/painless/.ssh/sshd_config &
...
#    lines=$(sort $HOST_FILE_PATH|uniq|wc -l)
  ...
  time mpirun -q --bind-to none --mca btl_tcp_if_include eth0 --allow-run-as-root -np ${AWS_BATCH_JOB_NUM_NODES} --hostfile combined_hostfile /painless/painless-mcomsps -c=8 -shr-sleep=750000 test.cnf 2> /dev/null
}
